...
def column_wise_resampling(x, replacement = False, random_state = 0, **options):
    ...
    x_decorrelation = np.zeros([n, p, s])
...
def decorrelation(x, solver = 'adam', hidden_layer_sizes = (100, 2), max_iter = 500, random_state = 0, clip_range = 0.9):
    """
    ...
    x = np.reshape(x, (n, -1))
    x_decorrelation = np.reshape(x_decorrelation, (n, -1))
